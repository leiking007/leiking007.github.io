...
```sql
-- 常规删除（如果用户下没有创建任何表或对象，可以直接删除）
DROP USER "KXCWT_TA404";

-- 级联删除（如果用户下已经创建了表、视图等对象，必须加上 CASCADE 强制删除）
DROP USER "KXCWT_TA404" CASCADE;
```
...
> 需要注意密码书写规范，window 和 linux 要求不一样；
>
> windows示例: 
>
> | 密码        | 连接串                           |
> | ----------- | -------------------------------- |
> | Pass"word01 | dexp user01/"""Pass""""word01""" |
> | Pass word01 | dexp user01/"Pass word01"        |
> | Pass/word01 | dexp user01/"""Pass/word01"""    |
>
> linux: 
...
# 导出
# SCHEMAS可以使用,分割填写多个
dexp USERID=SYSDBA/"""123456"""@ip:port SCHEMAS=KXCYB FILE=KXCYB.dmp DIRECTORY=E:\dmBak20260713


# 导入 
# 导入前需要先创建需要导入的模式（创建用户）
# 可以使用 REMAP_SCHEMA 将源模式中的数据导入到目标模式，格式：REMAP_SCHEMA=源SCHEMA:目标SCHEMA
dimp USERID=SYSDBA/"""123456"""@ip:port SCHEMAS=KXCYB FILE=KXCYB.dmp DIRECTORY=E:\dmBak20260713
```

### 查看数据库连接数

```sql
SELECT '当前总连接数' AS 指标, COUNT(*) AS 数量 FROM v$sessions 
UNION ALL
SELECT '当前活动连接数', COUNT(*) FROM v$sessions WHERE state = 'ACTIVE' 
UNION ALL
SELECT '当前空闲连接数', COUNT(*) FROM v$sessions WHERE state = 'IDLE' 
UNION ALL
SELECT '最大允许连接数', para_value FROM v$dm_ini WHERE para_name = 'MAX_SESSIONS';
```
...
- 查看归档路径及空间配置详情
  ...
  SELECT 
      ...
      ARCH_TYPE "归档类型",
      ARCH_DEST "归档目的地",
      ARCH_FILE_SIZE "单个归档文件大小(MB)",
      ARCH_SPACE_LIMIT "归档空间上限(MB)"
  FROM V$DM_ARCH_INI;
  ```

  通过 `ARCH_SPACE_LIMIT` 可以确认系统是否配置了自动清理归档日志的空间上限（0 表示无限制）。
...
- 设置归档空间上限
  ...
  ```sql
  -- 设置归档日志存储位置 /u01/dm8/dmdata/arch，本地归档，归档单个文件大小512M，最多占用50G
  ALTER DATABASE MODIFY ARCHIVELOG 'DEST=/u01/dm8/dmdata/arch, TYPE=LOCAL, FILE_SIZE=512, SPACE_LIMIT=51200';
  ```
