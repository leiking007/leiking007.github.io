...
#----------------------------------------------
# Author        : tanglei
# Description   : start yh_face with log output
#---------------------------------------------


SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
LOG_DIR="$SCRIPT_DIR/log"
LOG_FILE="$LOG_DIR/shlog.log"
...
# 1. 可修改区 -------------------------------------------------
PYTHON_EXEC=".runtime/python/install/bin/python"         # 也可以是 python、/usr/bin/python3 等
TARGET_SCRIPT="main.py"       # 你的 Python 入口脚本
PIDFILE=".pid"      # 存放进程号的文件，可放隐藏文件避免误删
# -------------------------------------------------------------
...
start() {
    if [[ -f "$PIDFILE" ]]; then
        ...
        if kill -0 "$PID" 2>/dev/null; then
            echo "Python 程序已在运行，PID=$PID" | tee -a "$LOG_FILE"
            return 1
        fi
        rm -f "$PIDFILE"
        echo "清理残留的 pidfile" | tee -a "$LOG_FILE"
    fi

    echo "正在启动 $TARGET_SCRIPT ..." | tee -a "$LOG_FILE"

    # 关键：双重脱离终端
    nohup "$PYTHON_EXEC" "$TARGET_SCRIPT" >> "$LOG_FILE" 2>&1 &
    PID=$!
    echo "$PID" > "$PIDFILE"
    ...
    if kill -0 "$PID" 2>/dev/null; then
        ...
        echo "启动失败，请查看日志" | tee -a "$LOG_FILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
clear_log(){
    # 启动时清空日志文件
    > "$LOG_FILE"
    echo "日志文件已清空" | tee -a "$LOG_FILE"
}

stop() {
    if [[ ! -f "$PIDFILE" ]]; then
        echo "找不到 pidfile，程序可能未启动" | tee -a "$LOG_FILE"
        return 1
    fi

    PID=$(cat "$PIDFILE")
    if kill -0 "$PID" 2>/dev/null; then
        echo "正在停止 PID=$PID ..." | tee -a "$LOG_FILE"
        kill "$PID"
        # 等待进程真正退出
        ...
        echo "进程 $PID 不存在，清理 pidfile" | tee -a "$LOG_FILE"
        rm -f "$PIDFILE"
    fi
}
...
status() {
    if [[ -f "$PIDFILE" ]]; then
        PID=$(cat "$PIDFILE")
        if kill -0 "$PID" 2>/dev/null; then
            echo "Python 程序运行中，PID=$PID" | tee -a "$LOG_FILE"
        else
            echo "进程 $PID 已退出，但 pidfile 仍存在" | tee -a "$LOG_FILE"
        fi
    else
        echo "Python 程序未运行" | tee -a "$LOG_FILE"
    fi
}
...
main(){
    ...
    echo "Operation: $1" | tee -a "$LOG_FILE"
...
#----------------------------------------------
# Author        : tanglei
# Description   : start jar package with separated logs
#---------------------------------------------


SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
LOG_DIR="$SCRIPT_DIR/log"
APP_LOG_FILE="$LOG_DIR/app.log"    # 程序日志
OPERATE_LOG_FILE="$LOG_DIR/operation.log"  # 操作日志
...
# 1. 可修改区 -------------------------------------------------
PIDFILE=".pid"      # 存放进程号的文件，可放隐藏文件避免误删
JAR_FILE="app.jar"  # JAR包文件名
JAVA_OPTS=""        # JVM参数，如: "-Xms512m -Xmx1024m"
JAR_OPTS="--server.port=8848 --spring.profiles.active=dev,websecurity,datasource-pgsql,cas,rate-limit,log,essc-zs,knife4j,wechat,app,api,face,intranetfaceauthen,yhagent --et.file.file_temp_path=/u01/soft/java/yhch-base/tmp"         # JAR包参数
# -------------------------------------------------------------
...
start() {
    ...
    if [[ -f "$PIDFILE" ]]; then
        PID=$(cat "$PIDFILE")
        if kill -0 "$PID" 2>/dev/null; then
            echo "JAR程序已在运行，PID=$PID" | tee -a "$OPERATE_LOG_FILE"
            return 1
        fi
        rm -f "$PIDFILE"
        echo "清理残留的 pidfile" | tee -a "$OPERATE_LOG_FILE"
    fi

    echo "正在启动 $JAR_FILE ..." | tee -a "$OPERATE_LOG_FILE"

    # 关键：彻底脱离终端，重定向所有输入输出
    nohup java $JAVA_OPTS -jar "$JAR_FILE" $JAR_OPTS >> "$APP_LOG_FILE" 2>&1 &
    PID=$!
    echo "$PID" > "$PIDFILE"
    ...
    sleep 3
    if kill -0 "$PID" 2>/dev/null; then
        echo "启动成功，PID=$PID" | tee -a "$APP_LOG_FILE"
    else
        echo "启动失败，请查看日志" | tee -a "$APP_LOG_FILE"
        rm -f "$PIDFILE"
        return 1
    fi
...
clear_log(){
    # 清空程序日志
    > "$APP_LOG_FILE"
    echo "程序日志文件已清空" | tee -a "$OPERATE_LOG_FILE"
}
...
stop() {
    ...
    PID=$(cat "$PIDFILE")
    if kill -0 "$PID" 2>/dev/null; then
        echo "正在停止 PID=$PID ..." | tee -a "$OPERATE_LOG_FILE"
        kill "$PID"
        # 等待进程真正退出
        while kill -0 "$PID" 2>/dev/null; do
            sleep 1
        ...
        rm -f "$PIDFILE"
        echo "已停止" | tee -a "$OPERATE_LOG_FILE"
    else
        echo "进程 $PID 不存在，清理 pidfile" | tee -a "$OPERATE_LOG_FILE"
        rm -f "$PIDFILE"
    fi
}

tail_log() {
    echo "查看程序日志..." | tee -a "$OPERATE_LOG_FILE"
    tail -500f "$APP_LOG_FILE"
}
...
status() {
    if [[ -f "$PIDFILE" ]]; then
        PID=$(cat "$PIDFILE")
        if kill -0 "$PID" 2>/dev/null; then
            echo "JAR程序运行中，PID=$PID" | tee -a "$OPERATE_LOG_FILE"
        else
            echo "进程 $PID 已退出，但 pidfile 仍存在" | tee -a "$OPERATE_LOG_FILE"
        fi
    else
        echo "JAR程序未运行" | tee -a "$OPERATE_LOG_FILE"
    fi
}

main(){
    # 记录脚本执行时间和操作到操作日志
    echo "------------------------------------------------" | tee -a "$OPERATE_LOG_FILE"
    echo "Script executed at: $(date)" | tee -a "$OPERATE_LOG_FILE"
    ...
    echo -n "please choose:"
    read num1
    echo "Operation: $num1" | tee -a "$OPERATE_LOG_FILE"
